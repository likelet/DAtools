...
public class FastQprocessConsole {
    ...
    public FastQprocessConsole(String[] args) throws IOException {
        if (args.length == 1) {
            System.out.println("Trim fastq by constant length :   \r\n\t\t" + ToolsforCMD.print_ansi_GREEN( "java -jar DAtools.jar -fastqP -mode trimedFastqFilter" )
                    + ToolsforCMD.print_ansi_CYAN( " <left> <right> <filterlength> <all.fq/gz> <passed.fq> <failed.fq> " ) + "\r\n");
            System.out.println("Trim fastq by constant length in memory <efficient mode (single end) :   \r\n\t\t" + ToolsforCMD.print_ansi_GREEN( "java -jar DAtools.jar -fastqP -mode EStrimedFastqFilter" )
                    + ToolsforCMD.print_ansi_CYAN( " <left> <right> <all.fq/gz> <out.fq> " ) + "\r\n");
            System.out.println("Trim fastq by constant length in memory efficient mode (paired end) :   \r\n\t\t" + ToolsforCMD.print_ansi_GREEN( "java -jar DAtools.jar -fastqP -mode EPtrimedFastqFilter" )
                    + ToolsforCMD.print_ansi_CYAN( " <left> <right> <fastq1/gz> <fastq2/gz> <failed.fq> " ) + "\r\n");
            System.out.println("Trim fastq by constant length only for those longer than x :   \r\n\t\t" + ToolsforCMD.print_ansi_GREEN( "java -jar DAtools.jar -fastqP -mode trimedFastqFilterByminiLength" )
                    + ToolsforCMD.print_ansi_CYAN( " <right> <lengthcondation> <all.fq/gz> <passed.fq> " ) + "\r\n");
            System.out.println("Trim fastq by constant length and retain constant reads :   \r\n\t\t" + ToolsforCMD.print_ansi_GREEN( "java -jar DAtools.jar -fastqP -mode trimZheng" )
